...
public class LevelHintController : MonoBehaviour
{
    ...
    public void LoadTexture(Texture2D shownLevelTexture2D, LevelEditorModel levelData, Vector2 spacing)
    {
        ...
        HintObject.transform.Find("Texture").GetComponent<RawImage>().texture = shownLevelTexture2D;

        // We also have to give the same size to the hint object.
        HintObject.transform.Find("Texture").GetComponent<RectTransform>().sizeDelta = new Vector2(
